...
Route::get('storage/images/{year}/{mounth}/{day}/{filename}', function ($year,$mounth,$day,$filename)
{
    $path = storage_path('app/public/images/'.$year .'/'. $mounth .'/'. $day .'/'. $filename );
...
Route::group(['prefix' => LaravelLocalization::setLocale(), 'middleware' => 'jailBanned'], function()
{
	...
	Route::group(['middleware' => ['auth', 'isVerified'], 'prefix' => 'account', 'as' => 'account.', 'namespace' => 'Account'], function()
	{
        ...
        Route::get('open_vendor', 'ProfileController@open_vendor')->name('open_vendor');
        
        Route::resource('purchase-history', 'PurchaseHistoryController');
        Route::get('purchase-history/download/{id}', 'PurchaseHistoryController@download');
        ...
        Route::get('settings/api', 'ApiController@index')->name('settings.api'); 
	...
	Route::group(['middleware' => ['auth', 'isVerified']], function () {
        ...
        Route::get('/inbox/show/{id}', 'InboxController@show');
...
Route::get('/sendmail','VoucherController@sendmail');
