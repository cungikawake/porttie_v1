...
class CreateController extends Controller
{
    ...
    public function store(Request $request)
    {
		...
		return response()->json(['listing'=>$listing], 200, ['X-IC-Redirect' => $listing->edit_url.'#details_section']);
    ...
    public function update($listing, Request $request)
    {
        ...
        if($request->input('discount') <= 100) {
            $listing->discount = $request->input('discount');
            $listing->save();
        }
        ...
        if($request->input('variants')) {
            foreach($params['variants'] as $variant_id => $variant) {
                ...
                if($listing_variant) {
                    ...
                    if($variant['max_guest'] > 0 ){
                        $listing_variant->max_guest = (int)$variant['max_guest'];
                    }else{
                        $listing_variant->max_guest = 0;
                    }
                    
                    $listing_variant->start_date = date('Y-m-d', strtotime($variant['start_date']));
                    $listing_variant->finish_date = date('Y-m-d', strtotime($variant['finish_date']));
                    $listing_variant->save();
        ...
        //set dates we don't want them to closed
        //dd($request->all());
        if($request->input('close_product')) {
            $count = 0;
            foreach($params['close_product'] as $position => $close_option) {
                if(!$close_option['close_date'])
                    continue;
                $listing_shipping_option = ListingProductClose::updateOrCreate([
                    'close_name'   => $close_option['close_name'],
                    'close_date'    => $close_option['close_date'],
            ...
            if($listing->close_options) {
                foreach ($listing->close_options as $position => $close_option) {
                    if ($position >= $count) {
                        $close_option->delete();
                    }
                }
