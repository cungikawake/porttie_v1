...
class RequestWidget extends AbstractWidget
{
    ...
    public function calculate_price($listing, $params) {
        ...
        $price_items = [
            [
                'key' 	=> 'price',
                'label' => __(':price x :quantity :unit_label', ['price' => format_money($listing_price, $listing->currency), 'quantity' => $quantity.' '.$listing->pricing_model->duration_name, 'unit_label' => $listing->unit]),
                'price' => $subtotal
            ],
            [
                'key'	=> 'service',
